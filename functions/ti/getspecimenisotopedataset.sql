CREATE OR REPLACE FUNCTION ti.getspecimenisotopedataset(_datasetid integer)
 RETURNS TABLE(specimenid integer, analysisunit character varying, depth text, thickness text, taxon character varying, element text, variable character varying, units character varying, value double precision, sd text, materialanalyzed character varying, substrate character varying, pretreatments text, analyst character varying, lab character varying, labnumber text, mass_mg text, weightpercent text, atomicpercent text, reps text)
 LANGUAGE sql
AS $function$
WITH speciesiso AS (
  SELECT
