CREATE OR REPLACE FUNCTION ap.dailyquerytable(_interval character varying)
 RETURNS TABLE(siteid integer, sitename character varying, datasetid integer, chronologyid integer, altitude double precision, datasettype character varying, databaseid integer, collectionunitid integer, colltype character varying, depenvt character varying, geog geography, older double precision, younger double precision, agetype character varying, publications integer[], taxa integer[], keywords integer[], contacts integer[], collectionunit jsonb, geopol integer[])
 LANGUAGE sql
AS $function$
WITH allids AS (
        SELECT st.siteid,
